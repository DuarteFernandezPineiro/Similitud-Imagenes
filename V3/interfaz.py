...
class SimilarityApp(tk.Tk):
    def __init__(self) -> None:
        ...
        self.folder_display = tk.StringVar(value="No se ha seleccionado ninguna carpeta")
    ...
    def _build_interface(self) -> None:
        ...
        self.folder_entry = ttk.Entry(setup_card, textvariable=self.folder_display, state="readonly", style="Path.TEntry")
        self.folder_entry.grid(row=2, column=0, sticky="ew", padx=(0, 10))
        source_actions = tk.Frame(setup_card, background=CARD_BACKGROUND)
        source_actions.grid(row=2, column=1, sticky="e")
        tk.Button(
            source_actions, text="Elegir carpeta", command=self._choose_folder, relief="flat", borderwidth=0,
        ...
        self.mobile_button = tk.Button(
            source_actions, text="Elegir dispositivo", command=self._choose_mobile, relief="flat", borderwidth=0,
            background=ACCENT_LIGHT, foreground="#1D4ED8", activebackground="#D8E6FF", activeforeground="#1D4ED8",
            disabledforeground="#8BA3C7", cursor="hand2", font=("Segoe UI", 10, "bold"), padx=13, pady=8,
        )
        self.mobile_button.pack(side="left", padx=(8, 0))
    ...
    def _choose_folder(self) -> None:
        ...
        if folder:
            if folder.startswith("::") or "usb#" in folder.lower():
                self._start_mobile_import(MobileDevice("Dispositivo conectado", folder))
                return
    ...
    def _choose_mobile(self) -> None:
        if self.analysis_running:
            return
        self.mobile_lookup_id += 1
        lookup_id = self.mobile_lookup_id
        self.mobile_button.configure(state="disabled")
        self.status.set("Buscando móviles conectados…")
        self.summary.set("Conecta el móvil, desbloquéalo y permite acceder a sus archivos.")
        ...
        def work() -> None:
            try:
                ...
                self.events.put(("mobile_devices", lookup_id, devices, None))
            except Exception as error:
                self.events.put(("mobile_devices", lookup_id, [], error))

        threading.Thread(target=work, daemon=True, name="buscar-moviles").start()

    def _show_mobile_devices(
        self, lookup_id: int, devices: list[MobileDevice], error: Exception | None
    ) -> None:
        if lookup_id != self.mobile_lookup_id:
            return
        self.mobile_button.configure(state="normal")
        self.progress.stop()
        if error:
            self.status.set("No se pudo buscar el móvil conectado.")
            messagebox.showerror("No se pudo abrir el móvil", str(error), parent=self)
            return
        if not devices:
            self.status.set("No se encontró ningún móvil conectado.")
            messagebox.showinfo(
                "No se encontró ningún móvil",
                "Conecta el móvil por cable, desbloquéalo y elige «Transferir archivos» en el teléfono.",
                parent=self,
            )
            return
        chooser = tk.Toplevel(self)
        chooser.title("Elegir dispositivo")
        chooser.transient(self)
        chooser.resizable(False, False)
        chooser.configure(background=CARD_BACKGROUND, padx=22, pady=18)
        tk.Label(chooser, text="Elige un dispositivo", background=CARD_BACKGROUND, foreground=TEXT_PRIMARY, font=("Segoe UI", 13, "bold")).pack(anchor="w")
        tk.Label(
            chooser,
            text="Se leerán las carpetas públicas de fotos y vídeos del móvil.\nLas imágenes se copiarán temporalmente para analizarlas con rapidez.",
            background=CARD_BACKGROUND, foreground=TEXT_SECONDARY, justify="left", font=("Segoe UI", 9),
        ).pack(anchor="w", pady=(4, 14))
        for device in devices:
            device_button = tk.Button(
                chooser, text=f"Dispositivo: {device.name}\nAnalizar galería pública del dispositivo",
                command=lambda selected_device=device: (chooser.destroy(), self._start_mobile_import(selected_device)),
                relief="flat", borderwidth=0, justify="left", anchor="w", background=ACCENT_LIGHT,
                foreground="#1D4ED8", activebackground="#D8E6FF", activeforeground="#1D4ED8",
                font=("Segoe UI", 10, "bold"), padx=16, pady=12, cursor="hand2", width=46,
            )
    ...
    def _start_mobile_import(self, device: MobileDevice, *, force_refresh: bool = False) -> None:
        ...
        self.mobile_button.configure(state="disabled")
        self.status.set(f"Preparando las fotos de {device.name}…")
        self.summary.set("Los vídeos se omitirán automáticamente.")
        self.progress.configure(mode="indeterminate", value=0)
        self.progress.start(12)
    ...
    def _finish_mobile_import(
        ...
        self.summary.set(f"{len(sources):,} fotos del móvil preparadas")
        self._start_analysis()
    ...
    def _consume_events(self) -> None:
        ...
        if latest_mobile_copy is not None:
            self._show_mobile_copy(*latest_mobile_copy)
        self.after(40, self._consume_events)
    ...
    def _set_navigation_state(self) -> None:
        ...
        busy = self.delete_in_progress
        self.previous_button.configure(state="normal" if not busy and reviewing_group and self.current_group > 0 else "disabled")
        can_go_next = reviewing_group and (
            self.current_group < len(self.groups) - 1 or self.pending_live_groups is not None
        )
        self.next_button.configure(state="normal" if not busy and can_go_next else "disabled")
        action_state = "normal" if reviewing_group and not busy else "disabled"
        self.select_all_button.configure(state=action_state)
        self.delete_button.configure(state=action_state)
        self.groups_button.configure(state="normal" if has_groups and not busy else "disabled")
    ...
    def _delete_current_selection(self) -> None:
        ...
        if not selected:
            ...
            return
        mobile_selected = {path: self.mobile_sources[path] for path in selected if path in self.mobile_sources}
        if mobile_selected:
            message = (
                f"Vas a eliminar definitivamente {len(mobile_selected)} imagen(es) del móvil conectado.\n\n"
                "La aplicación verificará que desaparezcan del dispositivo antes de actualizar los grupos. Esta acción no se puede deshacer."
            )
        else:
            message = (
        ...
        if mobile_selected:
            ...
            def work() -> None:
                ...
                try:
                    ...
                    for path, source in mobile_selected.items():
                        ...
                        try:
                            ...
                            pass  # La copia local no debe impedir reflejar el borrado del móvil.
                        deleted.add(path)
                    for path in local_selected:
                        try:
                            Path(path).unlink()
                            deleted.add(path)
                ...
                self.events.put(("mobile_deletion", group_id, deleted, failures))

            threading.Thread(target=work, daemon=True, name="eliminar-fotos-movil").start()
            return
